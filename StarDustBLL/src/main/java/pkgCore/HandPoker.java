...
public class HandPoker extends Hand implements Comparable {
	...
	public HandScore ScoreHand() throws HandException {
		...
		Collections.sort(super.getCards());   //collections is an abstract class 
	...
	private boolean isRoyalFlush() {
		...
		if ((isAce()==true)&&(isStraightFlush()==true)&&(this.getCards().get(1).geteRank()==eRank.KING))
		{
	...
	private boolean isFourOfAKind() {
		boolean bisFourOfAKind = false;//rows and frequency
